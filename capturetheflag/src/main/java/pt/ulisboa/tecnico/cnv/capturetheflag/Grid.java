...
public class Grid {
  ...
  public void spawnFlags(
      char placementType, List<int[]> redFlagPos, List<int[]> blueFlagPos, int numFlagsPerTeam) {
    switch (placementType) {
      case 'A':
        spawnFlagsTypeA(redFlagPos, blueFlagPos, numFlagsPerTeam);
        return;
      case 'B':
        spawnFlagsTypeB(redFlagPos, blueFlagPos, numFlagsPerTeam);
        return;
      case 'C':
        spawnFlagsTypeC(redFlagPos, blueFlagPos, numFlagsPerTeam);
        return;
      default:
        throw new IllegalArgumentException("Unknown flag type: " + placementType);
    }
  }
  ...
  private boolean isValidFlagSpot(int x, int y) {
    ...
    for (int dx = -1; dx <= 1; dx++) {
      for (int dy = -1; dy <= 1; dy++) {
        ...
        int nx = x + dx, ny = y + dy;
        if (inBounds(nx, ny)
            && (cells[nx][ny] == CellType.RED_FLAG || cells[nx][ny] == CellType.BLUE_FLAG)) {
          return false;
        }
      }
    }
  ...
  // ensure all neighbor cells around a flag are empty so that agents can always step adjacent to
  // flags
      ...
      for (int dy = -1; dy <= 1; dy++) {
        ...
        if (inBounds(nx, ny)
            && (cells[nx][ny] != CellType.EMPTY
                && cells[nx][ny] != CellType.RED_FLAG
                && cells[nx][ny] != CellType.BLUE_FLAG)) {
          cells[nx][ny] = CellType.EMPTY;
        }
      }
    }
  ...
  public void spawnAgents(
      List<Agent> blueAgents, List<Agent> redAgents, int num_blue_agents, int num_red_agents) {
    ...
    for (int i = 0; i < this.size; i++) {
      for (int j = 0; j < this.size; j++) {
        switch (this.cells[i][j]) {
          case EMPTY:
            sb.append(format(EMPTY));
            break;
          case OBSTACLE:
            sb.append(format(OBSTACLE));
            break;
          case BLUE_AGENT:
            sb.append(format(BLUE_AGENT));
            break;
          case BLUE_AGENT_ON_COOLDOWN:
            sb.append(format(BLUE_AGENT_ON_COOLDOWN));
            break;
          case BLUE_FLAG:
            sb.append(format(BLUE_FLAG));
            break;
          case BLUE_TRACE:
            sb.append(format(BLUE_TRACE));
            break;
          case RED_AGENT:
            sb.append(format(RED_AGENT));
            break;
          case RED_AGENT_ON_COOLDOWN:
            sb.append(format(RED_AGENT_ON_COOLDOWN));
            break;
          case RED_FLAG:
            sb.append(format(RED_FLAG));
            break;
          case RED_TRACE:
            sb.append(format(RED_TRACE));
            break;
        }
      }
      sb.append("\n");
