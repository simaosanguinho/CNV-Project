...
public abstract class AbstractJavassistTool implements ClassFileTransformer {
  ...
  private static List<BasicBlock> getBasicBlocks(CtBehavior behavior) throws BadBytecode {
    ...
    for (Block block :
        new ControlFlow(behavior.getDeclaringClass(), behavior.getMethodInfo()).basicBlocks()) {
      ...
      bbs.add(
          new BasicBlock(
              behavior,
              block.position(),
              ainfo.toLineNumber(block.position()),
              block.length(),
              entrances));
    }
  ...
  public final byte[] transform(String canonicalClassName) {
    byte[] bytecode = null;
    CtClass cc = null;
